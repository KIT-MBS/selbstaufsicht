...
class ShrinkedForcePermutationsDataset(torch.utils.data.Dataset):
    ...
    def split(self, validation_size: int, random: bool = True) -> Tuple[ShrinkedForcePermutationsDataset, ShrinkedForcePermutationsDataset]:
        ...
        if random:
            ...
            # <num_data_samples> might be smaller than the original dataset size -> add remaining samples to training dataset (they will not be used, unless <num_data_samples> is increased)
