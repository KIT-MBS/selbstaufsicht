...
def convert_tb_data(root_dir, sort_by='step', df_names=[]):
    """Convert local   oard data into a dict of Pandas DataFrames.
...
def parse():
    args = init()
    task_name, df_names = get_task_data(args)
    ...
    for idx, filename in enumerate(filenames):
        ...
        for name, df in all_df.items():
            ...
            if walltime < args.min_walltime or math.isnan(last_row['value']):
                data[name][-1]['value'] = 0.
            else:
